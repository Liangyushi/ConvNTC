...
class GetData(object):
    ...
    def __get_SimCombine_data__(self):
        ...
        for i in range(0,10):
            ...
            posIndex, poscv = torch.tensor(mmd[:, 5:], dtype=torch.int),torch.tensor(mmd[:, 0:5], dtype=torch.int)
            all_posIndex.append(posIndex)
            all_poscv.append(poscv)
            ###neg
            fname = os.path.join(self.folder, 'neg_mmd_{}neg_{}.txt'.format(self.neg,i+1))
            mmd = np.loadtxt(fname)
            negIndex, negcv = torch.tensor(mmd[:, 5:], dtype=torch.int), torch.tensor(mmd[:, 0:5], dtype=torch.int)
