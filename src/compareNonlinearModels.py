...
class NCTF_KAN_18(nn.Module):
    def __init__(self,shape, rank, nc, M, C, D, device, kernel_size=[], dims=[], act_func=nn.SiLU, dropout=0.5, input_dropout=0.2,alpha=0.5):
        ...
        # self.fc2 = nn.Linear(nc, 1)
        #self.output = KANLayers(self.input_size, self.dims, act_func, dropout, input_dropout)
        self.output = KANLayers(self.rank*len(shape), self.dims, act_func, dropout, input_dropout)
...
# class NCTF_ConvKAN_18_new(nn.Module):
#     def __init__(self,shape, rank, nc, M, C, D, device, kernel_size=[], dims=[], act_func=nn.SiLU, dropout=0.5, input_dropout=0.2,alpha=0.5):
#         super(NCTF_ConvKAN_18_new, self).__init__()
#         self.device = device
#         self.weight = [M, C, D]
#         self.rank = rank
#         self.shape = shape
...
#         # 这里将 NCTF 中 学习所得的 M C D 作为输入
#         #embeddings1 = [self.weight[i][x[i]] for i in range(len(x))]
#         e = [self.weight[i][x[i]] for i in range(len(x))]
#         e1 = self.fc1(e[0])
#         e2 = self.fc1(e[1])
#         e3 = self.fc2(e[2])
...
class NCTF_ConvMLP_18(nn.Module):
    def __init__(self,shape, rank, nc, M, C, D, device, kernel_size=[], dims=[], act_func=nn.ReLU(), dropout=0.5, input_dropout=0.2,alpha=0.5):
        super(NCTF_ConvMLP_18, self).__init__()
        self.device = device
        self.weight = [M, C, D]
        self.rank = rank
        self.shape = shape
...
class NCTF_ConvMax_18(nn.Module):
    def __init__(self,shape, rank, nc, M, C, D, device, kernel_size=[], dims=[], act_func=nn.ReLU(), dropout=0.5, input_dropout=0.2,alpha=0.5):
        super(NCTF_ConvMax_18, self).__init__()
        self.device = device
        self.weight = [M, C, D]
        self.rank = rank
        self.shape = shape
    ...
    def forward(self, x):
        ...
        ### 输出
        #print(out1.size())
        #out1 = torch.max(out1,dim=1).values
        ...
        #out2 = self.output(out2)
        #print(out2.size())
        #out2 = torch.max(out2,dim=1).values
...
class NCTF_MLP_18(nn.Module):
    def __init__(self,shape, rank, nc, M, C, D, device, kernel_size=[], dims=[], act_func=nn.ReLU, dropout=0.5, input_dropout=0.2,alpha=0.5):
        super(NCTF_MLP_18, self).__init__()
        self.device = device
        self.weight = [M, C, D]
        self.rank = rank
        self.shape = shape
        ...
        #self.output = KANLayers(self.input_size, self.dims, act_func, dropout, input_dropout)
        #self.output = KANLayers(self.rank*len(shape), self.dims, act_func, dropout, input_dropout)
        self.output = MLPlayers(self.rank*len(shape), self.dims, act_func, dropout, input_dropout)
...
class NCTF_ConvKAN_19(nn.Module):
    def __init__(self,shape, rank, M, C, D, device, sim=[], kernel_size=[], nc=[2,2],dims=[], act_func=nn.SiLU, dropout=0.5, input_dropout=0.2):
        super(NCTF_ConvKAN_19, self).__init__()
        self.device = device
        #self.weight = [M, C, D]
        self.rank = rank
        self.shape = shape
        ...
        self.sim = sim
        #self.embeds = nn.ModuleList([nn.Embedding(num_embeddings=shape[i], embedding_dim=rank) for i in range(len(shape))])
        self.embeds = nn.ModuleList([nn.Linear(shape[i], rank) for i in range(len(shape))])
        #self.featureFusion = nn.ModuleList([nn.Linear(rank*2,rank) for i in range(len(shape))])  # 用于特征融层
        self.conv1 = nn.Conv2d(in_channels=2, out_channels=self.channel[0], kernel_size=kernel_size[0], padding=0)
        self.conv2 = nn.Conv2d(in_channels=self.channel[0], out_channels=self.channel[1], kernel_size=kernel_size[1], padding=0)
        self.eca1 = eca_layer(2, 2) ## 加权两种不同通道的特征
    ...
    def forward(self, x):
        ###生成初始化的index embeddings
        #embeddings1 = [(self.embeds[i](x[i])) for i in range(len(x))]
        embeddings1 = [(self.embeds[i](self.sim[i][x[i]])) for i in range(len(x))]
        #print(embeddings1[0].size())
        ## 将 NCTF 中 学习所得的 秩一因子矩阵 M C D 作为第二种特征输入
        embeddings2 = [self.weight[i][x[i]] for i in range(len(x))]
        #print(embeddings2[0].size())
...
class NCTF_ConvKAN_20(nn.Module):
    def __init__(self,shape, rank, M, C, D, device, sim=[], kernel_size=[], nc=[2,2],dims=[], act_func=nn.SiLU, dropout=0.5, input_dropout=0.2):
        super(NCTF_ConvKAN_20, self).__init__()
        self.device = device
        #self.weight = [M, C, D]
        self.rank = rank
        self.shape = shape
        ...
        self.sim = sim
        self.embeds = nn.ModuleList([nn.Embedding(num_embeddings=shape[i], embedding_dim=rank) for i in range(len(shape))])
        self.embedSim = nn.ModuleList([nn.Linear(shape[i], rank) for i in range(len(shape))])
        #self.featureFusion = nn.ModuleList([nn.Linear(rank*2,rank) for i in range(len(shape))])  # 用于特征融层
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=self.channel[0], kernel_size=kernel_size[0], padding=0)
        self.conv2 = nn.Conv2d(in_channels=self.channel[0], out_channels=self.channel[1], kernel_size=kernel_size[1], padding=0)
        self.eca1 = eca_layer(3, 3) ## 加权两种不同通道的特征
        self.eca2 = eca_layer(self.channel[0], rank)
        self.flatten = nn.Flatten()
        self.act = nn.ReLU()
        #self.act = nn.LeakyReLU()
    ...
    def forward(self, x):
        ...
        ### 生成相似度嵌入
        embeddings3 = [(self.embedSim[i](self.sim[i][x[i]])) for i in range(len(x))]
        embeddings3 = [embeddings3[i].view(-1,1,self.rank) for i in range(len(x))]
        #print(embeddings3[0].size())
        concatenated3 = torch.cat(embeddings3,dim=1)
        #print(concatenated3.size())
        concatenated3 = concatenated3.view((concatenated1.size(0), 1, self.rank, len(self.shape)))
        
        out= torch.cat([concatenated1,concatenated2,concatenated3],dim=1) ## 将两种不同特征放于两个不同通道，形成两通道数据
        #print(out.size())
